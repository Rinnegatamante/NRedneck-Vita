                             ...
                             int8_t dashade, char dapal, const int32_t *daumost, const int32_t *dadmost)
{
    ...
    const int32_t cbackx = min(max(backx,(int32_t)0),daxsiz-1);
    const int32_t cbacky = min(max(backy,(int32_t)0),daysiz-1);
...
#endif
    {
        ...
        if (((a&1023) == 0) && (ysiz <= 256))  //vlineasm4 has 256 high limit!
        {
            ...
            for (x=x1; x<x2; x+=4)
            {
                ...
                bad = 15; xend = min(x2-x,(int32_t)4);
                for (xx=0; xx<xend; xx++)
                {
                    bx += xv2;
...
void renderDrawMapView(int32_t dax, int32_t day, int32_t zoome, int16_t ang)
{
    ...
    for (s=sortnum-1; s>=0; s--)
    {
        ...
        if ((spr->cstat&48) == 32)
        {
            ...
            globalshade = max(min(globalshade+spr->shade+6,(int32_t)numshades-1),(int32_t)0);
...
static void videoAllocateBuffers(void)
{
    ...
    const int32_t clamped_ydim = max(ydim, (int32_t)320);
...
int32_t videoSetGameMode(char davidoption, int32_t daupscaledxdim, int32_t daupscaledydim, int32_t dabpp, int32_t daupscalefactor)
{
    ...
    upscalefactor = max(1, (int)min(tabledivide32(yres, (int32_t)200), daupscalefactor));
